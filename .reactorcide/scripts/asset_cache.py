"""Small S3 asset-cache client for trusted Foundry CI code."""
...
LANE_PATTERN = re.compile(r"^[a-zA-Z0-9][a-zA-Z0-9_.-]{0,159}$")
...
class S3Cache:
    ...
    def from_environment(cls) -> "S3Cache":
        """Create a client without putting credentials in command arguments."""
        required = {
    ...
    def presign(self, method: str, key: str, *, expires: int = 43200) -> str:
        """Create one exact-object URL without exposing the secret key."""
...
def object_key(lane: str, asset: str) -> str:
    """Return one validated project, lane, and asset object key."""
...
def encode_manifest(value: dict[str, Any]) -> bytes:
    """Encode one stable manifest."""
    return (json.dumps(value, sort_keys=True, separators=(",", ":")) + "\n").encode()


def decode_manifest(content: bytes) -> dict[str, Any]:
    """Decode and validate the manifest shape."""
