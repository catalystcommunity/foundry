...
# $PKGS is intentionally unquoted: the default branch above builds a
# newline-separated package list from `go list`, which must word-split here.
# --integration drops -short so the testcontainers suite actually runs; without
# it every test under ./test/integration/... short-circuits on testing.Short().
if [[ "$DO_INTEGRATION" -eq 1 ]]; then
  step "go test (including Docker-backed integration suite)"
  # shellcheck disable=SC2086
  ( cd "$MODDIR" && go test $PKGS )
else
  step "go test -short"
  # shellcheck disable=SC2086
  ( cd "$MODDIR" && go test -short $PKGS )
fi
ok "unit tests"
