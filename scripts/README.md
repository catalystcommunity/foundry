...
| `scripts/test-local.sh --integration` | Runs the full suite **including** `./test/integration/...` (testcontainers), dropping `-short` so those tests actually execute. | go, Docker |

The default mode matches the CI contract in `.reactorcide/jobs/test.yaml`: gofmt,
vet, build, then `go test -short`. The integration suite is excluded two ways —
the package list skips it, and `-short` makes its `testing.Short()` guards skip
anyway.
