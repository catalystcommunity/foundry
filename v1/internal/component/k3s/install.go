...
// updateTLSSANConfig creates or updates the TLS SAN configuration file for K3s
// Returns true if the config was changed and certificates need regeneration
func updateTLSSANConfig(executor SSHExecutor, cfg *Config) (bool, error) {
	// Build the desired TLS SAN list
	desiredSANs := []string{}

	// Add all configured TLS SANs
	for _, san := range cfg.TLSSANs {
		if !contains(desiredSANs, san) {
			desiredSANs = append(desiredSANs, san)
		}
	}

	// Always add the VIP as a TLS SAN
	if cfg.VIP != "" && !contains(desiredSANs, cfg.VIP) {
		desiredSANs = append(desiredSANs, cfg.VIP)
	}
	...
	configLines = append(configLines, "# TLS Subject Alternative Names")
	configLines = append(configLines, "# Generated by foundry")
	configLines = append(configLines, "tls-san:")
	for _, san := range desiredSANs {
		configLines = append(configLines, fmt.Sprintf("  - \"%s\"", san))
	}
	newConfig := strings.Join(configLines, "\n") + "\n"

	// Ensure the config directory exists
	configPath := "/etc/rancher/k3s/config.yaml.d/tls-san.yaml"
	...
	result, err = executor.Exec(writeCmd)
	if err != nil {
		return false, fmt.Errorf("failed to write TLS SAN config: %w", err)
	}
	if result.ExitCode != 0 {
		return false, fmt.Errorf("failed to write TLS SAN config: exit code %d, stderr: %s", result.ExitCode, result.Stderr)
	}

	return true, nil
}

// regenerateK3sCertificates forces K3s to regenerate TLS certificates
// This is necessary when TLS SANs change
func regenerateK3sCertificates(executor SSHExecutor) error {
	fmt.Println("   Regenerating K3s certificates...")

	// Stop K3s
	result, err := executor.Exec("sudo systemctl stop k3s")
	if err != nil {
		return fmt.Errorf("failed to stop k3s: %w", err)
	}
	if result.ExitCode != 0 {
		return fmt.Errorf("failed to stop k3s: exit code %d, stderr: %s", result.ExitCode, result.Stderr)
	}

	// Delete TLS directory to force regeneration
	result, err = executor.Exec("sudo rm -rf /var/lib/rancher/k3s/server/tls")
	if err != nil {
		return fmt.Errorf("failed to delete TLS directory: %w", err)
	}
	if result.ExitCode != 0 {
		return fmt.Errorf("failed to delete TLS directory: exit code %d, stderr: %s", result.ExitCode, result.Stderr)
	}

	// Restart K3s (certificates will regenerate on startup)
	result, err = executor.Exec("sudo systemctl start k3s")
	if err != nil {
		return fmt.Errorf("failed to start k3s: %w", err)
	}
	if result.ExitCode != 0 {
		return fmt.Errorf("failed to start k3s: exit code %d, stderr: %s", result.ExitCode, result.Stderr)
	}

	// Wait for K3s to be ready after certificate regeneration
	if err := waitForK3sReady(executor, DefaultRetryConfig()); err != nil {
		return fmt.Errorf("k3s failed to become ready after certificate regeneration: %w", err)
	}
